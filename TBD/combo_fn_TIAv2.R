...
combo_fn <- function(region, site, cam){
  ...
  count <- read.csv(paste(cam, "TimelapseData.csv", sep = "/"), as.is = T) # Timelapse analyzer data
  ...
  tbl_df(rcnx)
  tbl_df(count)
  records <- inner_join(rcnx, count, by = c("Image.Name" = "File"))
  ...
  combo <- select(combo, -Image.Name, -Site, -Location, -Trigger, -Date.x, -Time.x, -Temp, 
                  ...
                  -Location.Longitude, -Folder, -Date.y, -Time.y, -X)
